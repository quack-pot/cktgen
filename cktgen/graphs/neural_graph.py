...
class NeuralGraph:
    ...
    def __init__(self, create_info: NeuralGraphCreateInfo) -> None:
        ...
        keep_mask: torch.Tensor = torch.ones(
            raw_node_count,
            dtype=torch.bool,
            device=create_info.edge_weights.device,
        )

        pruned_weights: torch.Tensor = create_info.edge_weights.clone()
        ...
        self.edge_weights: torch.Tensor = pruned_weights[keep_mask][:, keep_mask]
        self.biases: torch.Tensor = create_info.biases.clone()[keep_mask]

        self.total_node_count: int = self.biases.numel()
        self.hidden_node_count: int = self.total_node_count - self.input_node_count - self.output_node_count
    ...
    def renderGraph(self, filename: str) -> Digraph:
        ...
        for idx in range(self.total_node_count):
            for jdx in range(self.total_node_count):
                weight = float(self.edge_weights[idx, jdx].item())
                if weight == 0.0:
                    continue
